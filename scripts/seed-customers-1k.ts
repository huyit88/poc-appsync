/**
 * Customer Domain Seed Script (1K Customers)
 * 
 * Standalone script to seed 1,000 customers for performance testing.
 * Usage: npm run seed:customers:1k
 */
...
async function main() {
  try {
    console.log(`Starting seed of ${customerCount} customers...`);
    const startTime = Date.now();
    
    await seedCustomers(docClient, tableName, customerCount);
    
    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    console.log(`\n✓ Customer seeding completed successfully in ${duration}s`);
