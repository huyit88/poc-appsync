...
export function response(ctx) {
  ...
  if (error) {
    console.error('RDS Error:', JSON.stringify(error, null, 2));
    return util.appendError(error.message, error.type, result);
  }

  const jsonRes = JSON.parse(result);
  const sqlRes = jsonRes.sqlStatementResults;
  console.log('Raw sql:', JSON.stringify(sqlRes, null, 2));
  ...
  console.log('Transformed customer:', JSON.stringify(customer, null, 2));
  return customer;
}
