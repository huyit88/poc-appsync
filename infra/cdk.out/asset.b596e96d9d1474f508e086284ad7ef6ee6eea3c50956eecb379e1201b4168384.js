...
export function response(ctx) {
  ...
  for (const record of recordsToProcess) {
    ...
    // SQL query returns 12 columns: customer (5) + claims (7)
    if (Array.isArray(record) && record.length >= 12) {
      // Only process if claim_id exists (customer with claims)
      if (record[5] && record[5].stringValue) {
        ...
        claims.push({
          ...
          submittedAt: record[10] && record[10].stringValue ? record[10].stringValue : null,
          updatedAt: record[11] && record[11].stringValue ? record[11].stringValue : null,
        });
      }
    }
  }
