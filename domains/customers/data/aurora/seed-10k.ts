/**
 * Aurora Customer Seed Script (10K Customers)
 * 
 * Seeds 10,000 customer records into Aurora PostgreSQL for performance testing.
 * Uses the same customer data generation as DynamoDB seed for consistency.
 * 
 * Usage:
 *   tsx domains/customers/data/aurora/seed-10k.ts
 * 
...
async function seedCustomers() {
  console.log('Starting Aurora customer seed (10K)...');
  ...
  // Insert customers in batches - larger batch size for 10K
  ...
  for (let i = 0; i < customers.length; i += batchSize) {
    ...
    try {
      ...
      if (batchNum % progressInterval === 0 || batchNum === totalBatches) {
        ...
        console.log(`✓ Inserted batch ${batchNum}/${totalBatches} (${inserted.toLocaleString()}/${customers.length.toLocaleString()} customers, ${progress}%, ${elapsed}s)`);
      }
    } catch (error: any) {
  ...
  console.log('');
  console.log(`✓ Seeding completed: ${inserted.toLocaleString()}/${customers.length.toLocaleString()} customers inserted in ${duration}s (${minutes} minutes)`);
